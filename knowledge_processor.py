...
class FeatureLibrary:
    ...
    def from_file(cls, file_path: Path | None) -> "FeatureLibrary":
        ...
        # 核心修改：简化 YAML 解析逻辑，确保 keywords 和 link_domains 正常工作

        merged = DEFAULT_FEATURE_LIBRARY.copy()

        if "patterns" in raw:

            patterns = raw["patterns"] or {}
            ...
            #     "domains": patterns.get("link_domains", []),

            #     "cta_patterns": [],

            #     "line_patterns": patterns.get("keywords", []),

            #     "multiline_patterns": patterns.get("regex_blocks", []),

            # }

            # merged = {**DEFAULT_FEATURE_LIBRARY, **normalized}
...
def _is_ad_line(
    ...
    has_keyword = any(
        ...
        keyword in stripped for keyword in compiled_feature_library.feature_library.keywords

    )

    has_cta = any(pattern.search(stripped) for pattern in compiled_feature_library.cta_patterns)
...
def clean_directory(
    ...
    for md_path in _iter_markdown_files(directory, recursive=recursive):
        ...
        logger.info("写入净化结果: %s", md_path)

        _safe_print(f"[clean] 已清理: {md_path}")
